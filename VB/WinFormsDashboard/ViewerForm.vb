...
Namespace WinFormsDashboard
    Partial Public Class ViewerForm
        ...
        Private Sub dashboardViewer1_DashboardLoaded(ByVal sender As Object, ByVal e As DashboardLoadedEventArgs) Handles dashboardViewer1.DashboardLoaded
            ...
            For Each query In dataSources.SelectMany(Function(ds) ds.Queries.Select(Function(q) New With { _
                Key .DataSource = ds, _
                Key .Query = q _
            }))
                Dim ods = New DashboardObjectDataSource("ods|" & query.DataSource.ComponentName & "|" & query.Query.Name)
                e.Dashboard.DataSources.Add(ods)
                For Each item In e.Dashboard.Items.OfType(Of DataDashboardItem)().Where(Function(i) Object.ReferenceEquals(i.DataSource, query.DataSource) AndAlso i.DataMember = query.Query.Name)
                    item.DataMember = ""
                    item.DataSource = ods
                Next item
                For Each parameter In e.Dashboard.Parameters.Select(Function(p) p.LookUpSettings).OfType(Of DynamicListLookUpSettings)().Where(Function(p) Object.ReferenceEquals(p.DataSource, query.DataSource) AndAlso p.DataMember = query.Query.Name)
